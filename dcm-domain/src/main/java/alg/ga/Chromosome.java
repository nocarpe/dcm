...
public class Chromosome {
    public int[] gene=new int[excelData.keyInit().length];//基因序列  
     ...
     */  
    public Chromosome() {  
        //基因序列的值，0-9表示十个不同的地点，染色体初始化即将这十个值不重复的放进数组
        int[] a=excelData.keyInit();
        Random r=new Random();
        int max=a.length;
    ...
    public double getNum() {  
        ...
        for(int i=0;i<gene.length;i++)
        {
            ...
            sum+=excelData.backDis(y_temp, x_temp);
        }
        return sum;
    }
}  
